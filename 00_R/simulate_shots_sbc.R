...
#'
#' @export
simulate_nba_shots_sbc <- function(
  n_observations = 1000,
  n_defenders = 50,
  n_teams = 10,
  ...
  true_parameters <- list(
    b_Intercept = qlogis(0.4),  # Base log-odds of making a shot
    b_distance = -0.5,  # Effect of shot distance
    b_fouls = 0.02,  # Effect of fouls
    b_proximity = -0.1,  # Effect of defender proximity
    b_skill = -0.3,  # Effect of defensive skill
    sd_defender__Intercept = 0.5,  # Random effect standard deviation
    r_defender = matrix(rnorm(n_defenders, 0, 0.5), 
                       nrow = n_defenders, 
                       ncol = 1,
                       dimnames = list(1:n_defenders, "Intercept"))
  )
